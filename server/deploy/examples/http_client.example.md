...
```bash
# macOS / Linux — replace "My Vault" with your actual vault name, or omit if you have one vault:
...
- **"Connection refused"** — the server isn't running. Start it or install the background service.
- **"Port already in use"** — another process is on port 3001. Change it:
  - macOS / Linux: `OBSIDIAN_MCP_PORT=3002 obsidian-vaultgate-mcp`
  - Windows (PowerShell): `$env:OBSIDIAN_MCP_PORT=3002; obsidian-vaultgate-mcp`

  Then connect to `http://localhost:3002/mcp`.
